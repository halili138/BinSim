...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    H0b = JW_hamiltonian(mole, based=0, spin="aabb")
    svdgroups = compress_by_svd(H0b)
    agg = AGG(basis, svdgroups, mole.orbsym)
    ...
    hvec_direct_agg_benchmark!(basis, agg, H0b, dummy_src, dummy_dst, false) 
    println("Warm-up completed.")

    aop! = (src::Array{Float64,1}, dst::Array{Float64,1}) -> hvec_direct_agg_benchmark!(basis, agg, H0b, src, dst, true) 
