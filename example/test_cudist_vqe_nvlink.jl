...
include("../jl/cudistnetwork_nvlink.jl")
...
function main()
    ...
    funcs = CuDistributedFunctions(ModeNVLink, basis, ham, pool, comm; num_phases=_nphases)
    ...
    if rank == 0
        println("="^60)
        println("=== CuDistributedFunctions NVLink VQE Smoke Test ===")
        @printf("Command: mpiexec -n 2 julia --project=. example/test_cudist_vqe_nvlink.jl 1 h4 1.0 sto-3g 2\n")
