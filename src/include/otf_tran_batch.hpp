...
FORCE_INLINE Tv compute_coeff_tran(int b, int rank, Tv pa0, Tv pa1, const Tv *pan, const Tv *pb, int max_b_count)
{
    Tv vt = {};
    if constexpr (Rank == 1)
    {
...
#pragma omp simd reduction(+ : local_res)
                        ...
                        {
                            const Tv vt = compute_coeff_tran<Rank, 1, Tv>(b, rank, pa0, pa1, pan, pb, max_b_count);
...
#pragma omp critical
        {
            for (int64 g = 0; g < num_groups; ++g)
                trans[batch_groups[g].original_idx] += thread_trans[g];
        }
    }
}
...
#pragma omp simd reduction(+ : local_res)
                        ...
                        {
                            const Tv vt = compute_coeff_tran<Rank, 0, Tv>(b, rank, pa0, pa1, pan, pb0, max_b_count);
...
#pragma omp simd reduction(+ : local_res)
                        for (int b = 0; b < valid_nb; ++b)
                        {
                            const Tv vt = compute_coeff_tran<Rank, 0, Tv>(b, rank, pa0, pa1, pan, pb, max_b_count);
                            const int64 si = sa + sb_ptr[b];
                            const int64 di = da + db_ptr[b];
                            tran_update<Tv>(local_res, lp + si, lp + di, rp + si, rp + di, vt);
                        }
    ...
    Tv *trans)
{
    ...
    while (start < total_ngs)
    {
        ...
        {
            if (eff_rank == 1)
                tran_contract_diag_batched_impl<1>(basis, map, chunk, lp, rp, trans);
            else if (eff_rank == 2)
                tran_contract_diag_batched_impl<2>(basis, map, chunk, lp, rp, trans);
            else
                tran_contract_diag_batched_impl<0>(basis, map, chunk, lp, rp, trans);
        }
        else if constexpr (TypeCode == 1)
        {
            if (eff_rank == 1)
                tran_contract_pure_a_batched_impl<1>(basis, map, chunk, lp, rp, trans);
            else if (eff_rank == 2)
                tran_contract_pure_a_batched_impl<2>(basis, map, chunk, lp, rp, trans);
            else
                tran_contract_pure_a_batched_impl<0>(basis, map, chunk, lp, rp, trans);
        }
        else if constexpr (TypeCode == 2)
        {
            if (eff_rank == 1)
                tran_contract_pure_b_batched_impl<1>(basis, map, chunk, lp, rp, trans);
            else if (eff_rank == 2)
                tran_contract_pure_b_batched_impl<2>(basis, map, chunk, lp, rp, trans);
            else
                tran_contract_pure_b_batched_impl<0>(basis, map, chunk, lp, rp, trans);
        }
        else if constexpr (TypeCode == 3)
        {
            if (eff_rank == 1)
                tran_contract_mixed_batched_impl<1>(basis, map, chunk, lp, rp, trans);
            else if (eff_rank == 2)
                tran_contract_mixed_batched_impl<2>(basis, map, chunk, lp, rp, trans);
            else
                tran_contract_mixed_batched_impl<0>(basis, map, chunk, lp, rp, trans);
        }
        start = end;
    }
}
    ...
    Tv *__restrict__ trans)
{
    ...
    dispatch_tran_chunks_by_rank<0>(basis, net->map, net->pool_diag_jobs, lp, rp, trans);
    dispatch_tran_chunks_by_rank<1>(basis, net->map, net->pool_pure_a_jobs, lp, rp, trans);
    dispatch_tran_chunks_by_rank<2>(basis, net->map, net->pool_pure_b_jobs, lp, rp, trans);
    dispatch_tran_chunks_by_rank<3>(basis, net->map, net->pool_mixed_jobs, lp, rp, trans);
}
