...
struct SciBasisViewDev
{
    ...
    const Ti *dict_a_keys = nullptr; int *dict_a_vals = nullptr;
    const Ti *dict_b_keys = nullptr; int *dict_b_vals = nullptr;
    ...
    void clear()
    {
        ...
        if (dict_a_keys) { cudaFree(const_cast<Ti *>(dict_a_keys)); dict_a_keys = nullptr; }
        if (dict_a_vals) { cudaFree(dict_a_vals); dict_a_vals = nullptr; }
        if (dict_b_keys) { cudaFree(const_cast<Ti *>(dict_b_keys)); dict_b_keys = nullptr; }
        if (dict_b_vals) { cudaFree(dict_b_vals); dict_b_vals = nullptr; }
        if (is_new_a) { cudaFree(const_cast<bool *>(is_new_a)); is_new_a = nullptr; }
        if (is_new_b) { cudaFree(const_cast<bool *>(is_new_b)); is_new_b = nullptr; }
        if (candidate_diags) { cudaFree(const_cast<void *>(candidate_diags)); candidate_diags = nullptr; }
        dict_capacity = 0; host_block_num_a.clear(); host_block_num_b.clear();
    }
    SciBasisViewDev &operator=(SciBasisViewDev &&o) noexcept
    {
        if (this != &o) {
            clear();
            num_blocks = o.num_blocks; num_irreps = o.num_irreps; dim = o.dim;
            block_offsets = o.block_offsets; block_num_a = o.block_num_a; block_num_b = o.block_num_b;
            host_block_num_a = std::move(o.host_block_num_a);
            host_block_num_b = std::move(o.host_block_num_b);
            block_asym = o.block_asym; block_bsym = o.block_bsym;
            astrs_flat = o.astrs_flat; bstrs_flat = o.bstrs_flat;
            astrs_start = o.astrs_start; bstrs_start = o.bstrs_start;
            block_map = o.block_map;
            dict_a_keys = o.dict_a_keys; dict_a_vals = o.dict_a_vals;
            dict_b_keys = o.dict_b_keys; dict_b_vals = o.dict_b_vals;
            dict_capacity = o.dict_capacity;
            is_new_a = o.is_new_a; is_new_b = o.is_new_b;
            candidate_diags = o.candidate_diags;
            o.block_offsets = nullptr; o.block_num_a = nullptr; o.block_num_b = nullptr;
            o.block_asym = nullptr; o.block_bsym = nullptr;
            o.astrs_flat = nullptr; o.bstrs_flat = nullptr;
            o.astrs_start = nullptr; o.bstrs_start = nullptr;
            o.block_map = nullptr;
            o.dict_a_keys = nullptr; o.dict_a_vals = nullptr;
            o.dict_b_keys = nullptr; o.dict_b_vals = nullptr;
            o.dict_capacity = 0;
            o.is_new_a = nullptr; o.is_new_b = nullptr;
...
{
    int num_blocks = 0, num_irreps = 0; int64 dim = 0;
    const int64 *block_offsets = nullptr;
    const int *block_num_a = nullptr, *block_num_b = nullptr;
    const int *block_asym = nullptr, *block_bsym = nullptr;
    const Ti *astrs_flat = nullptr, *bstrs_flat = nullptr;
...
{
    capacity = 1ull; while (capacity < n * 2) capacity <<= 1;
...
{
    int64 ta = 0, tb = 0;
    for (int64 i = 0; i < hb->num_blocks; ++i) { ta += hb->blocks[i].num_a; tb += hb->blocks[i].num_b; }
    ...
    for (int64 i = 0; i < hb->num_blocks; ++i)
    {
        ...
        for (int a = 0; a < hb->blocks[i].num_a; ++a) { aka.push_back(hb->blocks[i].astrs[a]); ava.push_back((int)(a_base + a)); }
        for (int b = 0; b < hb->blocks[i].num_b; ++b) { bkb.push_back(hb->blocks[i].bstrs[b]); bvb.push_back((int)(b_base + b)); }
    }
    ...
    cudaFree(d_ika); cudaFree(d_iva); cudaFree(d_ikb); cudaFree(d_ivb);
    cudaFree(d_st);
}
    ...
    int &ni, int &nb)
{
    ...
    ho.resize(nb); hna.resize(nb); hnb.resize(nb); hasy.resize(nb); hbsy.resize(nb); has.resize(nb); hbs.resize(nb);
    int64 ta = 0, tb = 0;
    int bi = 0;
    for (int64 i = 0; i < hb->num_blocks; ++i) {
        if (filter_bid >= 0 && i != filter_bid) continue;
        const auto &b = hb->blocks[i];
        ho[bi] = (filter_bid >= 0) ? 0 : b.offset;
        hna[bi] = (int)b.num_a; hnb[bi] = (int)b.num_b;
        hasy[bi] = (int)b.asym; hbsy[bi] = (int)b.bsym;
        has[bi] = ta; hbs[bi] = tb;
        ta += hna[bi]; tb += hnb[bi]; ++bi;
    }
    haf.resize(ta); hbf.resize(tb);
    bi = 0;
    for (int64 i = 0; i < hb->num_blocks; ++i) {
        if (filter_bid >= 0 && i != filter_bid) continue;
        for (int j = 0; j < hb->blocks[i].num_a; ++j) haf[has[bi] + j] = hb->blocks[i].astrs[j];
        for (int j = 0; j < hb->blocks[i].num_b; ++j) hbf[hbs[bi] + j] = hb->blocks[i].bstrs[j];
        ++bi;
    }
}
...
static SciBasisViewDev<Ti> *upload_sci_src_basis(const SciBasisManager<Ti> *hb)
{
    ...
    db->num_blocks = nb; db->num_irreps = ni; db->dim = hb->dim;
    db->host_block_num_a = hna; db->host_block_num_b = hnb;
    db->block_offsets = up(ho.data(), nb);
    db->block_num_a = up(hna.data(), nb); db->block_num_b = up(hnb.data(), nb);
    db->block_asym = up(hasy.data(), nb); db->block_bsym = up(hbsy.data(), nb);
    db->astrs_flat = up(haf.data(), (int64)haf.size());
    db->bstrs_flat = up(hbf.data(), (int64)hbf.size());
    db->astrs_start = up(has.data(), nb); db->bstrs_start = up(hbs.data(), nb);

    std::vector<int> hbm(ni * ni, -1);
    for (int i = 0; i < nb; ++i) hbm[hasy[i] * ni + hbsy[i]] = i;
    ...
    const bool *is_new_a, const bool *is_new_b, const Tv *host_diags)
{
    ...
    db->num_blocks = nb; db->num_irreps = ni;
    ...
    if (is_new_a) { int64 o = blk.astrs - hb->all_astrs; db->is_new_a = up(is_new_a + o, blk.num_a); }
    if (is_new_b) { int64 o = blk.bstrs - hb->all_bstrs; db->is_new_b = up(is_new_b + o, blk.num_b); }
    if (host_diags) db->candidate_diags = (const void *)up(host_diags + blk.offset, blk.num_a * blk.num_b);
...
static SciBasisSliceDev<Ti, Tv> make_sci_slice(const SciBasisViewDev<Ti> *b)
{
    ...
    s.num_blocks = b->num_blocks; s.num_irreps = b->num_irreps; s.dim = b->dim;
    s.block_offsets = b->block_offsets; s.block_num_a = b->block_num_a; s.block_num_b = b->block_num_b;
    s.block_asym = b->block_asym; s.block_bsym = b->block_bsym;
    s.astrs_flat = b->astrs_flat; s.bstrs_flat = b->bstrs_flat;
    s.astrs_start = b->astrs_start; s.bstrs_start = b->bstrs_start;
    s.block_map = b->block_map;
    s.dict_a_keys = b->dict_a_keys; s.dict_a_vals = b->dict_a_vals;
    s.dict_b_keys = b->dict_b_keys; s.dict_b_vals = b->dict_b_vals;
    ...
    const Tv *__restrict__ sv, Tv *__restrict__ dv)
{
    ...
    while (s < groups.num_groups)
    {
        ...
        sl.num_groups = cs;
        sl.axs = groups.axs + s; sl.bxs = groups.bxs + s;
        sl.asyms = groups.asyms + s; sl.bsyms = groups.bsyms + s;
        sl.ranks = groups.ranks + s;
        sl.num_zas = groups.num_zas + s; sl.num_zbs = groups.num_zbs + s;
        sl.za_start = groups.za_start + s; sl.zb_start = groups.zb_start + s;
        sl.wa_start = groups.wa_start + s; sl.wb_start = groups.wb_start + s;
        sl.flat_zas = groups.flat_zas; sl.flat_zbs = groups.flat_zbs;
        sl.flat_wa = groups.flat_wa; sl.flat_wb = groups.flat_wb;
    ...
    int64 *__restrict__ out_count, int64 max_entries)
{
    ...
    if (tgt_basis.target_bids) return;
    const int n_a = tgt_basis.block_num_a[bid];
    const int n_b = tgt_basis.block_num_b[bid];
    ...
    for (int idx = threadIdx.x + blockIdx.y * blockDim.x; idx < n_a * n_b; idx += blockDim.x * gridDim.y)
    {
        ...
        if (acc == Tv{}) continue;
        if (!is_new_a[as + a] && !is_new_b[bs + b]) continue;
        const Tv haa = blk_diags[(int64)a * n_b + b];
        const Tv denom = e_var - haa;
        double dn_sq;
        if constexpr (std::is_arithmetic_v<Tv>) dn_sq = (double)(denom * denom);
        else dn_sq = (double)(denom.real() * denom.real() + denom.imag() * denom.imag());
        if (dn_sq == 0.0) continue;
        double a_sq;
        if constexpr (std::is_arithmetic_v<Tv>) a_sq = (double)(acc * acc);
        else a_sq = (double)(acc.real() * acc.real() + acc.imag() * acc.imag());
        if (a_sq / dn_sq <= eps_sq) continue;
        const int64 pos = atomicAdd((unsigned long long *)out_count, 1ull);
        if (pos < max_entries) { out_a[pos] = astrs[a]; out_b[pos] = bstrs[b]; out_v[pos] = acc; }
    }
}
...
int64 cuda_sci_select_external_block(
    ...
    int64 max_entries)
{
    if (blk_idx < 0 || blk_idx >= tgt_basis->num_blocks) return 0;
    const auto &blk = tgt_basis->blocks[blk_idx];
    if (blk.num_a <= 0 || blk.num_b <= 0) return 0;
    ...
    BasisSliceDev<Ti> ts;
    ts.num_blocks = 1; ts.num_irreps = tgt_dev->num_irreps; ts.dim = bdim;
    ts.block_offsets = tgt_dev->block_offsets;
    ts.block_num_a = tgt_dev->block_num_a; ts.block_num_b = tgt_dev->block_num_b;
    ts.block_asym = tgt_dev->block_asym; ts.block_bsym = tgt_dev->block_bsym;
    ts.astrs_flat = tgt_dev->astrs_flat; ts.bstrs_flat = tgt_dev->bstrs_flat;
    ts.astrs_start = tgt_dev->astrs_start; ts.bstrs_start = tgt_dev->bstrs_start;
    ...
    cudaFree(dv_dev); cudaFree(oc_dev);
    cudaFree(oa); cudaFree(ob); cudaFree(ov);
    cudaFree(sv_dev);
    delete tgt_dev;
    return oc;
}
    ...
    Tv *host_dst_vec)
{
    ...
    cudaFree(const_cast<Tv *>(src_dev)); cudaFree(dst_dev);
}

template <typename Ti>
static void destroy_sci_src_dev(SciBasisViewDev<Ti> *dev)
{
    if (dev) { dev->clear(); delete dev; }
}
