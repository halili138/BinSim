...
#pragma omp for collapse(2) schedule(static) nowait
        for (uint32 ia = 0; ia < R.n; ++ia)
        {
            for (int64 ib = 0; ib < nb; ++ib)
            {
                ...
                const int64 idx_src = PURE_A_IDX(blk_src, src_idx, ib);
                const int64 idx_dst = PURE_A_IDX(blk_dst, dst_idx, ib);
...
#pragma omp for collapse(2) schedule(static) nowait
        for (int64 ia = 0; ia < blk_src.num_a; ++ia)
        {
            for (uint32 ib = 0; ib < R.n; ++ib)
            {
                ...
                const Tv vo_fwd = co * vt;
                const Tv vo_rev = co * math_conj(vt);
                const int64 idx_src = PURE_B_IDX(blk_src, ia, src_idx);
                const int64 idx_dst = PURE_B_IDX(blk_dst, ia, dst_idx);
