...
template <typename Ti, typename Tv>
void expm_svd_sub_chunk_gpu_2d(const BasisViewDev<Ti> &basis, const NetworkDev<Ti, Tv> &net,
                               const SubTopologyDev &topo, int64 idx, double theta, Tv *dev_vec)
{
    BasisSliceDev<Ti> slice = make_dist_virtual_basis_slice(basis, topo);
    int virt_max_a = 0, virt_max_b = 0;
    for (int i = 0; i < slice.num_blocks; ++i)
    {
        if (slice.block_num_a[i] > virt_max_a)
            virt_max_a = slice.block_num_a[i];
        if (slice.block_num_b[i] > virt_max_b)
            virt_max_b = slice.block_num_b[i];
    }
    slice.max_a_count = virt_max_a;
    slice.max_b_count = virt_max_b;
    expm_svd_network_otf_gpu_2d<Ti, Tv>(slice, net, idx, theta, dev_vec);
}

template <typename Ti, typename Tv>
Tv backgrad_svd_sub_chunk_gpu_2d(const BasisViewDev<Ti> &basis, const NetworkDev<Ti, Tv> &net,
                                 const SubTopologyDev &topo, double theta, Tv *lp, Tv *rp)
{
    BasisSliceDev<Ti> slice = make_dist_virtual_basis_slice(basis, topo);
    int virt_max_a = 0, virt_max_b = 0;
    for (int i = 0; i < slice.num_blocks; ++i)
    ...
    slice.max_a_count = virt_max_a;
    slice.max_b_count = virt_max_b;
    return backgrad_svd_network_otf_gpu_2d<Ti, Tv>(slice, net, 0, theta, lp, rp);
}
