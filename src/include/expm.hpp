...
static FORCE_INLINE void expm_contract_otf_impl(
    const BasisManager<Ti> *basis, const SVDGroup_OTF<Ti, Tv> &group, double theta, Tv *vec)
{
...
static FORCE_INLINE void launch_expm_contract_group(
    ...
    Tv *vec)
{
    expm_contract_otf_impl<Rank, TypeCode>(basis, group, theta, vec);
}
