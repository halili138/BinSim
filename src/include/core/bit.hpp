...
struct uint256
{
    ...
    {
        if (lo < o.lo) return {lo - o.lo, hi - o.hi - 1UL};
...
template <> FORCE_INLINE uint128 get_zero<uint128>() { return 0; }
template <> FORCE_INLINE uint256 get_zero<uint256>() { return {0, 0}; }
template <typename Ti> FORCE_INLINE Ti get_one();
template <> FORCE_INLINE uint128 get_one<uint128>() { return 1; }
template <> FORCE_INLINE uint256 get_one<uint256>() { return {1, 0}; }

template <typename T> struct half_width;
template <> struct half_width<uint16> { using type = uint8; };
template <> struct half_width<uint32> { using type = uint16; };
template <> struct half_width<uint64> { using type = uint32; };
template <> struct half_width<uint128> { using type = uint64; };
template <> struct half_width<uint256> { using type = uint128; };
template <typename T> using half_width_t = typename half_width<T>::type;

template <typename T> struct double_width;
template <> struct double_width<uint8> { using type = uint16; };
template <> struct double_width<uint16> { using type = uint32; };
template <> struct double_width<uint32> { using type = uint64; };
template <> struct double_width<uint64> { using type = uint128; };
template <> struct double_width<uint128> { using type = uint256; };
template <typename T> using double_width_t = typename double_width<T>::type;
...
{
    std::size_t operator()(__uint128_t const &x) const noexcept { return fold_for_hash(x); }
};

template <>
struct std::hash<uint256>
{
    std::size_t operator()(uint256 const &x) const noexcept { return fold_for_hash(x); }
};
...
struct ankerl::unordered_dense::hash<__uint128_t>
{
    ...
    auto operator()(__uint128_t const &x) const noexcept -> uint64_t { return fold_for_hash(x); }
};

template <>
struct ankerl::unordered_dense::hash<uint256>
{
    using is_avalanching = void;
    auto operator()(uint256 const &x) const noexcept -> uint64_t { return fold_for_hash(x); }
};
