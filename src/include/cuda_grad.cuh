...
__global__ void grad_diag_kernel_2d(
...
template <int Rank, typename Ti, typename Tv>
__global__ void grad_mixed_kernel_2d(
    const BasisSliceDev<Ti> basis,
    const GroupsSliceDev<Ti, Tv> groups,
...
template <int Rank, typename Ti, typename Tv>
__global__ void grad_pure_a_kernel_2d(
    const BasisSliceDev<Ti> basis,
    const GroupsSliceDev<Ti, Tv> groups,
    int pos, double cd, double co,
    const Tv *__restrict__ lp,
...
template <int Rank, typename Ti, typename Tv>
__global__ void grad_pure_b_kernel_2d(
    const BasisSliceDev<Ti> basis,
    const GroupsSliceDev<Ti, Tv> groups,
    int pos, double cd, double co,
    const Tv *__restrict__ lp,
    ...
    const Tv *__restrict__ lp, const Tv *__restrict__ rp)
{
    ...
    case 0: // Diag
    {
        ...
        else
        {
            ...
            if (rank == 1)
                grad_diag_kernel_2d<1, Ti, Tv><<<grid, block>>>(basis_slice, slice, pos, theta, lp, rp, d_res);
            else if (rank == 2)
                grad_diag_kernel_2d<2, Ti, Tv><<<grid, block>>>(basis_slice, slice, pos, theta, lp, rp, d_res);
            else
                grad_diag_kernel_2d<0, Ti, Tv><<<grid, block>>>(basis_slice, slice, pos, theta, lp, rp, d_res);
            break;
        }
    }
    case 1:
    {
        ...
        if (rank == 1)
            grad_pure_a_kernel_2d<1, Ti, Tv><<<grid, block>>>(basis_slice, slice, pos, cd, co, lp, rp, d_res);
        else if (rank == 2)
            grad_pure_a_kernel_2d<2, Ti, Tv><<<grid, block>>>(basis_slice, slice, pos, cd, co, lp, rp, d_res);
        else
            grad_pure_a_kernel_2d<0, Ti, Tv><<<grid, block>>>(basis_slice, slice, pos, cd, co, lp, rp, d_res);
        break;
    }
    case 2:
    {
        ...
        if (rank == 1)
            ...
            grad_pure_b_kernel_2d<2, Ti, Tv><<<grid, block>>>(basis_slice, slice, pos, cd, co, lp, rp, d_res);
        else
            grad_pure_b_kernel_2d<0, Ti, Tv><<<grid, block>>>(basis_slice, slice, pos, cd, co, lp, rp, d_res);
        break;
    }
    case 3:
    {
        ...
        if (rank == 1)
            grad_mixed_kernel_2d<1, Ti, Tv><<<grid, block>>>(basis_slice, slice, pos, cd, co, lp, rp, d_res);
        else if (rank == 2)
            grad_mixed_kernel_2d<2, Ti, Tv><<<grid, block>>>(basis_slice, slice, pos, cd, co, lp, rp, d_res);
        else
            grad_mixed_kernel_2d<0, Ti, Tv><<<grid, block>>>(basis_slice, slice, pos, cd, co, lp, rp, d_res);
        break;
    }
    default:
        break;
