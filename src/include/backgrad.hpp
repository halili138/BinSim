...
static FORCE_INLINE Tv backgrad_contract_diag_otf_impl(
...
template <int Rank, typename Ti, typename Tv>
static FORCE_INLINE Tv backgrad_contract_pure_a_otf_impl(
    const BasisManager<Ti> *basis, const SVDGroup_OTF<Ti, Tv> &group,
    double theta, Tv *lp, Tv *rp)
{
...
template <int Rank, typename Ti, typename Tv>
static FORCE_INLINE Tv backgrad_contract_pure_b_otf_impl(
    const BasisManager<Ti> *basis, const SVDGroup_OTF<Ti, Tv> &group,
    double theta, Tv *lp, Tv *rp)
{
    const double ecd = std::cos(theta) - 1.0;
...
template <int Rank, typename Ti, typename Tv>
static FORCE_INLINE Tv backgrad_contract_mixed_otf_impl(
    const BasisManager<Ti> *basis, const SVDGroup_OTF<Ti, Tv> &group,
    double theta, Tv *lp, Tv *rp)
{
    const double ecd = std::cos(theta) - 1.0;
...
Tv backgrad_svd_network_otf(const BasisManager<Ti> *basis, const Network_OTF<Ti, Tv> *net, int64 idx, double theta, Tv *lp, Tv *rp)
{
    ...
    case 0:
        if (rank == 1)
            res = backgrad_contract_diag_otf_impl<1>(basis, group, theta, lp, rp);
        else if (rank == 2)
            res = backgrad_contract_diag_otf_impl<2>(basis, group, theta, lp, rp);
        else
            res = backgrad_contract_diag_otf_impl<0>(basis, group, theta, lp, rp);
        break;
    case 1:
        if (rank == 1)
            res = backgrad_contract_pure_a_otf_impl<1>(basis, group, theta, lp, rp);
        else if (rank == 2)
            res = backgrad_contract_pure_a_otf_impl<2>(basis, group, theta, lp, rp);
        else
            res = backgrad_contract_pure_a_otf_impl<0>(basis, group, theta, lp, rp);
        break;
    case 2:
        if (rank == 1)
            res = backgrad_contract_pure_b_otf_impl<1>(basis, group, theta, lp, rp);
        else if (rank == 2)
            res = backgrad_contract_pure_b_otf_impl<2>(basis, group, theta, lp, rp);
        else
            res = backgrad_contract_pure_b_otf_impl<0>(basis, group, theta, lp, rp);
        break;
    case 3:
        if (rank == 1)
            res = backgrad_contract_mixed_otf_impl<1>(basis, group, theta, lp, rp);
        else if (rank == 2)
            res = backgrad_contract_mixed_otf_impl<2>(basis, group, theta, lp, rp);
        else
            res = backgrad_contract_mixed_otf_impl<0>(basis, group, theta, lp, rp);
        break;
    }
