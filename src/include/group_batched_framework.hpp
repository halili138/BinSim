...
// Shared batched traversal for multiple OTF groups.  Rank and TypeCode remain
// compile-time parameters so the diagonal/pure-a/pure-b/mixed branches are
// pruned by if constexpr, while Op supplies the concrete math/update policy.
template <int Rank, int TypeCode, typename Ti, typename Tv, typename Op>
static FORCE_INLINE void group_contract_batched_impl(
...
static FORCE_INLINE void dispatch_group_chunks_by_rank(
    const BasisManager<Ti> *basis,
    ...
    const Op &op)
{
    ...
    while (start < total_ngs)
    {
        ...
        {
        case 1:
            group_contract_batched_impl<1, TypeCode>(basis, chunk_ptr, chunk_size, op);
            break;
        case 2:
            group_contract_batched_impl<2, TypeCode>(basis, chunk_ptr, chunk_size, op);
            break;
        default:
            group_contract_batched_impl<0, TypeCode>(basis, chunk_ptr, chunk_size, op);
            break;
        }
