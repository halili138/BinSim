...
#include "cuda/basis.cuh"
#include "cuda/otf.cuh"
...
template <int Rank, int TypeCode, typename Ti, typename Tv>
__global__ void grad_single_group_kernel_2d(
    const BasisSliceDev<Ti> basis,
    const GroupsSliceDev<Ti, Tv> groups,
    int pos,
    double theta,
...
template <int TypeCode, typename Ti, typename Tv>
static inline void launch_single_group_grad_2d(
    const BasisSliceDev<Ti> &basis_slice,
    const GroupsSliceDev<Ti, Tv> &groups,
    int64 pos,
    double theta,
    ...
    int host_rank)
{
    ...
    if (host_rank == 1)
        grad_single_group_kernel_2d<1, TypeCode, Ti, Tv><<<grid, block>>>(basis_slice, groups, pos, theta, cd, co, lp, rp, d_res);
    else if (host_rank == 2)
        grad_single_group_kernel_2d<2, TypeCode, Ti, Tv><<<grid, block>>>(basis_slice, groups, pos, theta, cd, co, lp, rp, d_res);
    else
        grad_single_group_kernel_2d<0, TypeCode, Ti, Tv><<<grid, block>>>(basis_slice, groups, pos, theta, cd, co, lp, rp, d_res);
}

template <typename Ti, typename Tv>
Tv grad_svd_network_otf_gpu_2d(
    const BasisViewDev<Ti> &basis,
    const NetworkDev<Ti, Tv> &net,
    int64 idx, double theta,
    const Tv *__restrict__ lp,
    const Tv *__restrict__ rp)
{
    ...
    case 0:
    {
        ...
        else
        {
            ...
            launch_single_group_grad_2d<0, Ti, Tv>(basis_slice, slice, pos, theta, lp, rp, d_res, rank);
            break;
        }
    }
    case 1:
    {
        ...
        launch_single_group_grad_2d<1, Ti, Tv>(basis_slice, slice, pos, theta, lp, rp, d_res, rank);
        break;
    }
    case 2:
    {
        ...
        launch_single_group_grad_2d<2, Ti, Tv>(basis_slice, slice, pos, theta, lp, rp, d_res, rank);
        break;
    }
    case 3:
    {
        ...
        launch_single_group_grad_2d<3, Ti, Tv>(basis_slice, slice, pos, theta, lp, rp, d_res, rank);
        break;
    }
    default:
        break;
