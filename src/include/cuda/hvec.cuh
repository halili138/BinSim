    ...
    Tv *__restrict__ dst_vec)
{
    ...
    while (start < total_ngs)
    {
        ...
        else
        {
            ...
            {
            case 1:
                hvec_gather_offdiag_kernel<1, TypeCode, Ti, Tv><<<grid_size, block_size>>>(basis_slice, slice, src_vec, dst_vec);
                break;
            case 2:
                hvec_gather_offdiag_kernel<2, TypeCode, Ti, Tv><<<grid_size, block_size>>>(basis_slice, slice, src_vec, dst_vec);
                break;
            default:
                hvec_gather_offdiag_kernel<0, TypeCode, Ti, Tv><<<grid_size, block_size>>>(basis_slice, slice, src_vec, dst_vec);
                break;
            }
        }
    ...
    Tv *__restrict__ diags)
{
    ...
    const int bid = basis.target_bids ? basis.target_bids[blockIdx.x] : blockIdx.x;
    constexpr int SHARED_MEM_SIZE = BATCH_GROUP_SHARED_MEM<0>;
    ...
    for (int task_idx = blockIdx.y; task_idx < total_tiles; task_idx += gridDim.y)
    {
        ...
        for (int sh_idx = threadIdx.x; sh_idx < current_tile_b; sh_idx += blockDim.x)
        {
            ...
            compute_phase_dev<0, Ti, Tv>(bstr, zbs, num_zb, wb, sh_pb + sh_idx, TILE_B, rank);
        }
        ...
        {
            Tv pa[128] = {};
            compute_phase_dev<0, Ti, Tv>(astr, zas, num_za, wa, pa, 1, rank);
...
#pragma unroll
                for (int b_offset = 0; b_offset < TILE_B; ++b_offset)
                {
                    const Tv vt = compute_coeff_dev<0, Tv>(pa, sh_pb, TILE_B, rank, b_offset);
    ...
    Tv *__restrict__ diags)
{
    ...
    get_diags_elements_kernel<Ti, Tv><<<grid_size, block_size>>>(basis_slice, slice, diags);
