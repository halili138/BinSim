...
Tv grad_svd_network_otf(const BasisManager<Ti> *basis, const Network_OTF<Ti, Tv> *net, int64 idx, double theta, const Tv *lp, const Tv *rp)
{
    ...
    case 0:
        if (rank == 1)
            res = grad_contract_diag_otf_impl<1>(basis, group, theta, lp, rp);
        else if (rank == 2)
            res = grad_contract_diag_otf_impl<2>(basis, group, theta, lp, rp);
        else
            res = grad_contract_diag_otf_impl<0>(basis, group, theta, lp, rp);
        break;
    case 1:
        if (rank == 1)
            res = grad_contract_pure_a_otf_impl<1>(basis, group, theta, lp, rp);
        else if (rank == 2)
            res = grad_contract_pure_a_otf_impl<2>(basis, group, theta, lp, rp);
        else
            res = grad_contract_pure_a_otf_impl<0>(basis, group, theta, lp, rp);
        break;
    case 2:
        if (rank == 1)
            res = grad_contract_pure_b_otf_impl<1>(basis, group, theta, lp, rp);
        else if (rank == 2)
            res = grad_contract_pure_b_otf_impl<2>(basis, group, theta, lp, rp);
        else
            res = grad_contract_pure_b_otf_impl<0>(basis, group, theta, lp, rp);
        break;
    case 3:
        if (rank == 1)
            res = grad_contract_mixed_otf_impl<1>(basis, group, theta, lp, rp);
        else if (rank == 2)
            res = grad_contract_mixed_otf_impl<2>(basis, group, theta, lp, rp);
        else
            res = grad_contract_mixed_otf_impl<0>(basis, group, theta, lp, rp);
        break;
    }
