...
#pragma omp for collapse(2) schedule(static) nowait
        for (uint32 ia = 0; ia < R.n; ++ia)
        {
            for (int64 ib = 0; ib < nb; ++ib)
            {
                ...
                const double r0 = rp[idx_src];
                const double r1 = rp[idx_dst];
                res += lp[idx_src] * (r0 * vd + r1 * vo) +
                       lp[idx_dst] * (r1 * vd - r0 * vo);
            }
        }
    }
