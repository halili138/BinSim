...
function initial_te_buffer(dim::Int, N::Int;
    method::String="ite", mode::String="adjoint", Tv::DataType=Float64,
)
    @assert method in ["ite", "rte"],
    @assert mode in ["adjoint", "forward"]

    if (method, mode) == ("ite", "adjoint")
        return TimeEvolBuffer(
            Vector{Vector{Tv}}(),
            Vector{Vector{Tv}}(),
            [zeros(Tv, dim) for _ in 1:5],
            Vector{Vector{Float64}}(),
            zeros(Float64, N, N),
            zeros(Float64, N),
        )
    elseif (method, mode) == ("rte", "adjoint")
        return TimeEvolBuffer(
            Vector{Vector{Tv}}(),
            Vector{Vector{Tv}}(),
            [zeros(Tv, dim) for _ in 1:5],
            [zeros(Float64, N) for _ in 1:5],
            zeros(Float64, N, N),
            zeros(Float64, N),
        )
    elseif (method, mode) == ("ite", "forward")
        return TimeEvolBuffer(
            [zeros(Tv, dim) for _ in 1:N],
            [zeros(Tv, dim) for _ in 1:N],
            [zeros(Tv, dim) for _ in 1:5],
            Vector{Vector{Float64}}(),
            zeros(Float64, N, N),
    ...
    elseif (method, mode) == ("rte", "forward")
        return TimeEvolBuffer(
            [zeros(Tv, dim) for _ in 1:N],
            [zeros(Tv, dim) for _ in 1:N],
            [zeros(Tv, dim) for _ in 1:5],
...
function _run_vqite_tfim_adjoint_diag(
    ...
    dt::Float64=0.01, max_step::Int=500, xtol::Float64=1e-6, epsilon::Float64=1e-4,
    per_print::Int=10, verbose::Int=1, run_rk4::Bool=true,
) where {Tv}
    ...
    time_ops = @elapsed for step in 1:max_step
        run_rk4 && rk4_step!(f_hvec, ve, vt, ws, -1.0, dt)
        ...
        fid = run_rk4 ? abs2(dot(v, ve)) / vnorm : 0.0
...
function run_adapt_vqite_tfim_adjoint(
    ...
    println("============================================================================")
    println("--- Adaptive VQITE (Outer: ADAPT Select | Inner: VQITE Relax compared with RK4) ---")
    ...
    zero_grads = zeros(Tv, length(pool))
    converged = false
    iter = 0
    amplitudes = Float64[]
    selec_idxs = Int64[]
