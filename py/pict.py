...
def plot_speedup_ratio(save_filename="speedup_chart.png"):
    ...
    output_file = os.path.join(current_dir, f"pict/{save_filename}")
    ...
    # 绘制每一组柱子
    rects1 = ax.bar(
        x - 1.5 * width, ratio_9a14_96t, width, label="EPYC-9A14-96t", color="#2ca02c"
    )
    rects2 = ax.bar(
        x - 0.5 * width, ratio_9a14_192t, width, label="EPYC-9A14-192t", color="#d62728"
    )
    rects3 = ax.bar(
        x + 0.5 * width, ratio_a100, width, label="A100-80G", color="#1f77b4"
    )
    rects4 = ax.bar(
        x + 1.5 * width, ratio_h100, width, label="H100-80G", color="#ff7f0e"
    )
    ...
    print(f"速度倍率图已成功保存为: {output_file}")
    plt.close(fig)
